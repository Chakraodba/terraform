...
resource "azurerm_network_security_group" "example" {
  name                = "allow_port_80_only_external"
  location            = azurerm_resource_group.main.location
  resource_group_name = azurerm_resource_group.main.name
...
resource "azurerm_network_security_rule" "custome1" {
  name                        = "allow_22"
  priority                    = 110
  direction                   = "Inbound"
  access                      = "Allow"
...
resource "azurerm_network_interface_backend_address_pool_association" "example" {
  ...
  ip_configuration_name   = azurerm_network_interface.main1.ip_configuration[0].name
  backend_address_pool_id = azurerm_lb_backend_address_pool.bpepool.id
}
...
resource "azurerm_nat_gateway" "example" {
  ...
  location                = azurerm_resource_group.main.location
  resource_group_name     = azurerm_resource_group.main.name
  public_ip_address_ids   = [azurerm_public_ip.nats.id]
  sku_name                = "Standard"
...
resource "azurerm_public_ip" "nats" {
